...
DATASETS = {
    'c4': lambda: load_dataset('json', data_files={'train': 'calib_data/c4/c4-train.00000-of-01024.json'}, trust_remote_code=True),
    'code-alpaca':lambda: load_dataset('json', data_files={'train': 'calib_data/evol-alpaca/train.jsonl'}, trust_remote_code=True)
...
def get_calib_dataloder(
    ...
    if 'code' in dataset:
        def format_conversation(examples):
            """Convert instruction-output format to conversation text"""
            formatted_texts = []
            for instruction, output in zip(examples['instruction'], examples['output']):
                # Convert to conversation format following ChatDataset style
                conversation = f"Human: {instruction}\n\nAssistant: {output}"
                formatted_texts.append(conversation)
            return {"text": formatted_texts}

        # First format data as text
        calib_set = calib_set.map(
            format_conversation,
            batched=True,
            remove_columns=calib_set.column_names,
        )
    ...
    if n_blocks_for_stat > 0:
        if len(lm_calib_set) > n_blocks_for_stat:
            ...
            lm_calib_set = lm_calib_set.select(range(len(lm_calib_set)))
